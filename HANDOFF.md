...
>
> _2026-09-12, evening:_ **two feed fixes.** The keyword pages failed with a Postgres « malformed array literal »: postgrest-js serialises `.contains(col, [v])` without quotes and the `t:` / `c:` colon broke the array literal — the filter is now a pre-quoted string, and an unknown keyword id short-circuits to an empty page. And Boursenews bodies were scraped without their **lead paragraph** (a separate `mini_list` block above the body), so a one-sentence flash showed only its last line: the scraper now prepends the lead, and a small migration resets the cached `body_*` columns so articles refetch whole. **One more migration to run:** see §12.
>
...
`src/lib/sectors.ts` now carries all **80 listings across 15 sectors**, keyed by the exchange's code (post-alias, so Disway is `DWY`), with the workbook's own company name in a comment on every line so the two can still be compared by eye. It sits beside `CSE_SYMBOLS` because it is the same kind of thing: a constant of the product that moves by a few lines a year, not something to administer. `sectorKey()` returns a `Key`, so a sector added without its French _and_ English label fails to compile.

**The grouping is the owner's, from the « Secteur Action Lyamfi » workbook, not the exchange's official nomenclature.** The workbook folds beverages into Agro-alimentaire, breaks Automobile out of Distribution, files the property vehicles (and Risma) under OPCI, and keeps a deliberate _Autres_ catch-all instead of a bucket per orphan. That is a product taxonomy built for a legible filter, and it is the one that governs here. Do not "correct" it toward the BVC's own list.
...
- The chips are sorted by their **translated** label, so their order changes with the language — except _Autres_, pinned last by `CATCH_ALL_SECTOR`, because a catch-all sitting at its letter between two named sectors reads as a sixteenth sector.
...
## 9b. Trilingual French / English / Arabic
...
- **RTL is structural, not cosmetic.** Flex and grid mirror themselves with `dir="rtl"`; text alignment was swept to logical utilities (`text-start` / `text-end`) across the hand-written pages, and the directional icons (back links, CTA arrows) carry `rtl:rotate-180`. Physical spacing utilities (`ml-*`, `pl-*`, `left-*`…) were deliberately left alone this round, and charts stay LTR; see §10.
- **Arabic numbers keep Western digits.** `NUMBER_LOCALE` maps `ar-MA` to `ar-MA-u-nu-latn`, which groups with the comma (`100,000 MAD`). The comma also dodges a real bidi trap: a space-grouped number in an RTL paragraph renders its groups reversed (« 000 100 » for 100 000), so the static strings in `ar.ts` use the same comma grouping. `MAD` leads the amount in Arabic, as in English.
- **Font:** IBM Plex Sans Arabic, loaded beside Inter in `__root.tsx` and applied under `html[lang="ar"]`; the headings' negative letter-spacing is disabled there because it breaks Arabic joining.
- **The translation was glossary-bound and reviewed.** Moroccan financial press register throughout: مكرر الربحية for PER, ربحية السهم for BPA, توزيعات السهم for DPA, الرسملة السوقية for capitalisation, رقم المعاملات for chiffre d'affaires, هيكل المساهمين for actionnariat, مستمر / التثبيت for the quotation modes. Pure-symbol ratios (P/B, P/S, P/FCF, PEG, ROE, YTD) and acronyms (MAD, BVC, MASI, OPCI) stay Latin. The TradingView widgets receive `ar` as their locale.
...
**Then run `supabase/migrations/20260912220000_bnews_body_chapeau_refetch.sql`** (same day, evening): it resets the cached Boursenews bodies — and their translations, which derive from them — so each article is scraped again, this time with the lead paragraph the first pass dropped (§9f). A plain `UPDATE`, idempotent, nothing to check afterwards.
