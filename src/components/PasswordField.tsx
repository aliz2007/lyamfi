    ...
    <div>
      ...
      <div className="relative mt-1.5">
        <input
          ...
          className={`w-full rounded-xl border bg-background py-3 pl-4 pr-12 text-sm outline-none focus:border-primary ${
...
export function PasswordRules({
  ...
  value: string;
  check: {
    results: { rule: { id: string; label: string }; ok: boolean }[];
    strength: number;
  ...
  return (
    <div className="rounded-xl border border-border bg-muted/30 p-4">
      <div className="flex items-center justify-between gap-3">
        <p className="text-xs font-medium">Ton mot de passe doit contenir :</p>
        {value.length > 0 && (
          <span className="text-xs text-muted-foreground">{check.strengthLabel}</span>
        )}
      </div>
      ...
      <ul className="mt-3 space-y-1.5">
        {check.results.map(({ rule, ok }) => (
          <li key={rule.id} className="flex items-center gap-2 text-xs">
            ...
            <span className="sr-only">{ok ? "— condition remplie" : "— condition manquante"}</span>
          </li>
        ))}
      </ul>
      ...
      {check.tooLong && (
        <p className="mt-3 text-xs text-destructive">
          Trop long : {max} caractères maximum.
        </p>
      )}
    </div>
  );
